...
Totals: **600 choice questions + 180 LLM questions** across 60 modules.
...
- Exactly 10 `choices`, exactly 3 `llm`.
- `kind` ∈ {`mcq`,`msq`}; `mcq` ⇒ exactly 1 correct, `msq` ⇒ ≥2 correct.
- Each choice has ≥3 options; every `correct_answers` entry must appear in
  `choices`.
...
- 10 choice questions: deliberate MCQ/MSQ mix, plausible distractors, difficulty
  spread (recall → application), no trick/ambiguous items, exactly-correct keys.
...
- **Question quality at scale (780 items).** Mitigated by the mandatory review
